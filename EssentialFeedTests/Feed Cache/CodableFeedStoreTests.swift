    ...
    func test_retrieveAfterInsertingToEmptyCache_deliversInsertedValues(){
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { retrieveResult in
                ...
                default:
                    XCTFail("Expected found result with feed \(feed) and timesstamp \(timestamp) got \(retrieveResult) instead")
                }
                exp.fulfill()
            }
        }
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                sut.retrieve { secondResult in
                    ...
                    default:
                        XCTFail("Expected retrieving teice from non empty cache to deliver same found result with feed \(feed) and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead")
                    }
                    exp.fulfill()
                }
            }
