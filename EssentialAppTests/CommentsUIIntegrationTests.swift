...
//  EssentialAppTests
//
//  Created by Iris GalGal on 24/04/23.
//
...
class CommentsUIIntegrationTests: FeedUIIntegrationTest{
    ...
    override func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator for first view once first image  loading complete successfully")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no loading indicator for second view once first image  loading complete successfully")
      
        loader.completeImageLoadingWithError(at: 0)
        XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected no loading indicator state change for first view once second image  loading completes with error")
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected no loading indicator for second view once second image  loading complete with error")
    }
    ...
    private func makeSUT(currentDate: @escaping () -> Date = Date.init, file: StaticString = #file, line:UInt = #line) -> (sut: ListViewController, loader: LoaderSpy){
